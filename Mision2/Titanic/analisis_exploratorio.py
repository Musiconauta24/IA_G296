...
titanic=pd.read_csv('Mision2/Titanic/titanic.csv')
...
#Mostrar porpantalla las filas pares del dataframe
print(titanic.iloc[range(0,titanic.shape[0],2)])

# Mostrar los nombres de las personas que iban en primera clase ordenados alfabeticamente
print (titanic.loc[titanic['Pclass']==1]['Name'].sort_values())

# Mostrar el porcentaje de sobrevivientes que luego murieron
print (titanic['Survived'].value_counts()/titanic['Survived'].count())

# Mostrar el porcentaje de sobrevivientes de cada clase
print (titanic.groupby('Pclass')['Survived'].value_counts(normalize = True))

# Mostrar la edad promedio de mujeres de cada clase
...
print(titanic.groupby(['Pclass', 'Age'])['Age'].mean().unstack()['female'])